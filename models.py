...
def fcn32(vgg16, l2=0):
    ...
    Returns:
      (keras Model)
    '''
    x = keras.layers.Conv2D(filters=5, kernel_size=(1,1), strides=(1,1), padding='same', activation='linear',
                            kernel_regularizer=keras.regularizers.L2(l2=l2),
                            name='score7')(vgg16.get_layer('drop7').output)
    x = keras.layers.Conv2DTranspose(filters=5, kernel_size=(64,64), strides=(32,32),
...
def crossentropy(y_true, y_pred_onehot):
    ...
    y_true_onehot = tf.cast(np.arange(5) == y_true, tf.float32)
