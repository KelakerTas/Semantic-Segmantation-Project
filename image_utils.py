...
def rotate(im, angle, scale, interpolation, cval=0):
    ...
    return cv2.warpAffine(im, mat, (new_w, new_h), flags=interpolation, borderValue=(255, 255, 255))
...
def random_rotate(im, gt, rotation_range, zoom_range, ignore_label):
    ...
    y = rotate(gt, angle, scale=zoom, interpolation=cv2.INTER_NEAREST)
    return x, y
